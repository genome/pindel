...
private:
  ...
  const std::string
  makeNiceLine(const std::string & rawDescription) const;
  const std::string
  describeTab() const
...
StringParameter::StringParameter(std::string * par_ptr,
...
Parameter::Parameter(const std::string & shortName,
    const std::string & longName, const std::string & description,
    const bool required)
{
  d_required = required;
...
bool
readTransgressesBinBoundaries(SPLIT_READ & read,
...
defineParameters(std::string & WhichChr)
{
  ...
  parameters. push_back(
      new IntParameter(&par.numThreads, "-T", "--number_of_threads",
          "the number of threads Pindel will use (default 1).", false, 1));
  ...
  parameters. push_back(
      new FloatParameter(&Seq_Error_Rate, "-e", "--sequencing_error_rate",
  ...
  parameters. push_back(
      new IntParameter(&MIN_IndelSize_NT, "-n", "--min_NT_size",
          "only report inserted (NT) sequences in deletions greater than this size "
            "(default 50)", false, 50));
  parameters. push_back(
      new IntParameter(&MIN_IndelSize_Inversion, "-v", "--min_inversion_size",
              ...
              for (unsigned int i = 0; i < bams_to_parse.size(); i++)
                {
                  ...
                  ReturnFromReadingReads = ReadInBamReads(
                      bams_to_parse[i].BamFile.c_str(), WhichChr, &CurrentChr,
                      Reads, bams_to_parse[i].InsertSize, bams_to_parse[i].Tag,
                  ...
                  std::cout << "BAM file index\t" << i << "\t" << Reads. size()
                      << std::endl;
                }
          ...
          if (All_BD_events.size() > 1)
            {
                  ...
                  if (!Reads[ReadIndex].UP_Far.empty())
                    { // if a far end has been found
                      ...
                      if (Reads[ReadIndex]. UP_Far[Reads[ReadIndex].UP_Far.size()
                          - 1].LengthStr + Reads[ReadIndex].CloseEndLength
                          < Reads[ReadIndex].ReadLength)
                        {
              ...
              // TODO: Ask Kai whether this can be removed
                  ...
                  << CountFarEndMinus << std::endl << std::endl; //endl;
              delete[] BD_INDEX;
            }
...
#pragma omp critical
                      ...
                      if (Reads[ReadIndex].MatchedD == Plus)
                      ...
                      else if (Reads[ReadIndex].MatchedD == Minus)
                        {
                          if (Reads[ReadIndex]. UP_Close[Reads[ReadIndex].UP_Close.size()
                              - 1].AbsLoc > Reads[ReadIndex].UP_Far[0].AbsLoc
                              + MIN_IndelSize_Inversion)
                            {
                              for (short MAX_SNP_ERROR_index = 0; MAX_SNP_ERROR_index
                                  <= Reads[ReadIndex].MAX_SNP_ERROR; MAX_SNP_ERROR_index++)
                                      ...
                                      == Plus)
                                    {
                                      if (Reads[ReadIndex].UP_Far[FarIndex]. LengthStr
                                              ...
                                              >= Min_Num_Matched_Bases)
                                        {
                                          ...
                                          Reads[ReadIndex].BPLeft
                                              = Reads[ReadIndex].UP_Close[CloseIndex].AbsLoc
                                                  + 1 - SpacerBeforeAfter; // - Fragments[LeftReads[Left_Index].MatchedSeqID].Start;
                                          Reads[ReadIndex].BPRight
                                              = Reads[ReadIndex].UP_Far[FarIndex].AbsLoc
                                                  - SpacerBeforeAfter; // - Fragments[LeftReads[Left_Index].MatchedSeqID].Start;
                                          // TODO: Ask Kai whether this can be removed
              ...
              for (int Index = 0; Index < TotalNumReads; Index++)
                {
                  CloseEndMappedOutput << Reads[Index].Name << "\n"
                      ...
                      << Reads[Index].MatchedD << "\t" << Reads[Index].FragName
                      << "\t" << Reads[Index].MatchedRelPos << "\t"
                      << Reads[Index].MS << "\t" << Reads[Index].InsertSize
                      << "\t" << Reads[Index].Tag << "\n";
                }
            }
          ...
          std::cout << "Total: " << TotalNumReads << ";\tClose_end_found "
              ...
              << ";\tUsed\t" << Count_Used << "." << std::endl << std::endl;
...
ReverseComplement(const std::vector<std::string> &InputPatterns)
{
...
ReverseComplement(const std::string & InputPattern)
{
  ...
  unsigned int LenPattern = InputPattern.size();
  for (unsigned int j = 0; j < LenPattern; j++)
    OutputPattern[j] = Convert2RC4N[(unsigned int) InputPattern[LenPattern - j
        - 1]];
  return OutputPattern;
}
...
bool
NotInVector(const std::string & OneTag,
...
void
GetRealStart4Insertion(const std::string & TheInput,
...
void
GetFarEnd_OtherStrand(const std::string & CurrentChr,
    SPLIT_READ & Temp_One_Read, const short &RangeIndex)
{
  // TODO: Ask Kai whether this can be removed
...
void
GetFarEnd_BothStrands(const std::string & CurrentChr,
    SPLIT_READ & Temp_One_Read, const short &RangeIndex)
{
  // TODO: Ask Kai whether this can be removed
  //short ReadLength = Temp_One_Read.UnmatchedSeq.size();
...
void
CheckBoth(const SPLIT_READ & OneRead, const std::string & TheInput,
          ...
          if (Sum)
            {
              ...
              CheckBoth(OneRead, TheInput, CurrentReadSeq, PD_Plus_Output,
                  PD_Minus_Output, BP_Start, BP_End, CurrentLengthOutput, UP);
            }
          else
            return;
        }
