...
int main (int argc, char *argv[]) {
   ...
   while (argv1==0 && cin >> TempOne.QNAME || argv1!=0 && inf_Read >> TempOne.QNAME)
   {
      ...
      if (TempOne.MRNM == "=") TempOne.MRNM = TempOne.RNAME;
...
bool WhetherN(const string & input) {
  for (unsigned int i = 0; i < input.size(); i++)
     if (input[i] == 'N') return true;
                                                    
   return false;
}

void GetFlag(READ & input) {
   if (input.FLAG % 2 == 1) input.Paired = true;
        else input.Paired = false;
        if ((input.FLAG / 2) % 2 == 1) input.ProperPaired = true;
        else input.ProperPaired = false;
        if ((input.FLAG / 4)  % 2 == 1) input.QueryUnmapped = true;
        else input.QueryUnmapped = false;
        if ((input.FLAG / 8)  % 2 == 1) input.MateUnmapped = true;
        else input.MateUnmapped = false;
        if ((input.FLAG / 16)  % 2 == 1) input.StrandOfQuery = ReverseStrand;
        else input.StrandOfQuery = ForwardStrand;
        if ((input.FLAG / 32)  % 2 == 1) input.StrandOfMate = ReverseStrand;
        else input.StrandOfMate = ForwardStrand;
        if ((input.FLAG / 64)  % 2 == 1) input.FirstRead = true;
        else input.FirstRead = false;
        if ((input.FLAG / 128) % 2 == 1) input.SecondRead = true;
        else input.SecondRead = false;
        if ((input.FLAG / 256) % 2 == 1) input.NotPrimaryAlignment = true;
        else input.NotPrimaryAlignment = false;
        if ((input.FLAG / 512) % 2 == 1) input.FailQualityCheck = true;
        else input.FailQualityCheck = false;
        if ((input.FLAG / 1024) % 2 == 1) input.Duplication = true;
        else input.Duplication = false;
}
...
short WhetherReport(const READ & input) {
   ...
   for (int i = 0; i < input.SEQ.size(); i++) {
      if (input.SEQ[i] == 'N') NumN++;
   }
   if (NumN * 10 > input.SEQ.size()) return 0;
   if (input.QueryUnmapped == true && input.MateUnmapped == false) return 1; //  one-end mapped
