...
short AssembleOneSV(const std::vector <Chromosome> & AllChromosomes, std::map<std::string,int> &ChrName2Index, ControlState & CurrentState, ParCollection & par, const Assembly & OneSV, std::ofstream & ASM_Output) {
    ...
    for (unsigned ReadIndex = 0; ReadIndex < First.size(); ReadIndex++) {
        ...
        std::cout << "First UP_Far: " << First[ReadIndex].UP_Far.size() << "\t" << "First UP_Far_backup: " << First[ReadIndex].UP_Far_backup.size() << std::endl;
        if (First[ReadIndex].UP_Far.size()) {
            ...
            std::cout << "First UP_Far_backup " << First[ReadIndex].UP_Far_backup[First[ReadIndex].UP_Far_backup.size() - 1].LengthStr << " " << First[ReadIndex].UP_Close[First[ReadIndex].UP_Close.size() - 1].LengthStr << " " << Max_NT_Size << " " << First[ReadIndex].ReadLength << std::endl;
    ...
    for (unsigned ReadIndex = 0; ReadIndex < Second.size(); ReadIndex++) {
        Second[ReadIndex].FarFragName = OneSV.ChrB;
        ...
        std::cout << "Second UP_Far: " << Second[ReadIndex].UP_Far.size() << "\t" << "Second UP_Far_backup: " << Second[ReadIndex].UP_Far_backup.size() << std::endl;
        if (Second[ReadIndex].UP_Far.size()) {
            ...
            std::cout << "Second UP_Far_backup" << std::endl;
            std::cout << "Second UP_Far_backup " << Second[ReadIndex].UP_Far_backup[Second[ReadIndex].UP_Far_backup.size() - 1].LengthStr << " " << Second[ReadIndex].UP_Close[Second[ReadIndex].UP_Close.size() - 1].LengthStr << " " << Max_NT_Size << " " << Second[ReadIndex].ReadLength << std::endl;
...
void OutputCurrentRead(const std::vector <Chromosome> & AllChromosomes, std::map<std::string,int> & ChrName2Index, ControlState & CurrentState, ParCollection & par, const Assembly & OneSV, SPLIT_READ & OneRead, std::ofstream & ASM_Output) {
    std::cout << "OutputCurrentRead" << std::endl;
    if (OneRead.UP_Far.size()) {
        ASM_Output << OneSV.Index << " " << OneSV.Type << " " << OneSV.ChrA << " " << OneSV.PosA << " " << OneSV.CI_A << "\t" << OneSV.ChrB << " " << OneSV.PosB << " " << OneSV .CI_B 
        << "\tA " << OneRead.MatchedD << " " << OneRead.MS << " " << OneRead.Thickness << "\t" << OneRead.FragName  
        ...
        << " " << OneRead.UP_Close[OneRead.UP_Close.size() - 1].AbsLoc - g_SpacerBeforeAfter 
        ...
        << " " << OneRead.UP_Far[OneRead.UP_Far.size() - 1].AbsLoc - g_SpacerBeforeAfter 
        << std::endl;
    }
        ...
        ASM_Output << OneSV.Index << " " << OneSV.Type << " " << OneSV.ChrA << " " << OneSV.PosA << " " << OneSV.CI_A << "\t" << OneSV.ChrB << " " << OneSV.PosB << " " << OneSV .CI_B 
            ...
            << " " << OneRead.UP_Far_backup[OneRead.UP_Far_backup.size() - 1].AbsLoc - g_SpacerBeforeAfter << "\t" << "NT_Size: " << NT_Size << "\tNT_Str: " << NT_Str
            << std::endl;        
    }
}
